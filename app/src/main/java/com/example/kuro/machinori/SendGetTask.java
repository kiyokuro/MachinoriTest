        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    HttpGet httpGet = new HttpGet("http://133.242.228.233/pos-test.php?latitude="+latitude+"&longitude="+longitude+"&accelerationX="+
                        accelerationX+"&accelerationY="+accelerationY+"&accelerationZ="+accelerationZ+"&deviceId="+deviceId);
                    HttpResponse httpResponse = httpClient.execute(httpGet);
                    String str = EntityUtils.toString(httpResponse.getEntity(), "UTF-8");
                    //TextView respons = (TextView)gpsActivity.findViewById(R.id.response);
                    //respons.setText(str);
                    Log.d("ÅöHTTP", str);
                } catch(Exception ex) {
